<x-layouts.app title="Désignations — OptiGest">
...
<div style="max-width:90%; margin:0 auto;">
    ...
    <div style="display:flex; align-items:center; gap:16px; margin-bottom:28px;">
        ...
        <div>
            <h1 style="font-size:26px; font-weight:800; color:#0f172a; margin:0;">Désignations</h1>
            <p style="font-size:15px; color:#94a3b8; margin:6px 0 0;">Noms normalisés des articles (Progressifs, Solaires, Ray-Ban Clubmaster…)</p>
        </div>
    </div>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:24px; align-items:start;">
        ...
        <div style="background:#fff; border-radius:18px; border:1px solid #bfdbfe;
            ...
            <h2 style="font-size:18px; font-weight:700; color:#0f172a; margin:0 0 20px;">Ajouter une désignation</h2>
            <form method="POST" action="{{ route('designations.store') }}">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="nom" value="{{ old('nom') }}" required maxlength="255"
                           placeholder="Ex: Progressifs, Clubmaster, Acuvue Oasys…"
        ...
        <div style="background:#fff; border-radius:18px; border:1px solid #bfdbfe;
            ...
            <h2 style="font-size:18px; font-weight:700; color:#0f172a; margin:0 0 16px;">
                Liste ({{ $designations->total() }})
            </h2>
            @forelse($designations as $d)
            <div style="display:flex; align-items:center; justify-content:space-between;
                        padding:12px 0; border-bottom:1px solid #f0f7ff;">
                <div>
                    <p style="font-size:15px; font-weight:700; color:#0f172a; margin:0;">{{ $d->nom }}</p>
                    <p style="font-size:13px; color:#94a3b8; margin:2px 0 0;">{{ $d->produits_count }} article(s)</p>
                </div>
                @if($d->produits_count === 0)
                <form method="POST" action="{{ route('designations.destroy', $d) }}"
                      onsubmit="return confirm('Supprimer « {{ $d->nom }} » ?')" style="margin:0;">
                    @csrf @method('DELETE')
                    <button type="submit"
                ...
                @else
                <span style="font-size:12px; color:#94a3b8; font-style:italic;">utilisée</span>
                @endif
            </div>
            @empty
            <p style="color:#94a3b8; font-size:15px; text-align:center; padding:20px 0;">Aucune désignation enregistrée</p>
            @endforelse

            @if($designations->hasPages())
            <div style="margin-top:16px;">{{ $designations->links() }}</div>
            @endif
        </div>
